...
>(
  ...
  if (errors.length) {
    ...
    errors.forEach(e =>
      console.error(
        `- Field "${e.path?.[e.path.length - 1]?.key}" - ${e.message}`,
      ),
    );
  }
  if (warnings.length) {
    ...
    if (process.env.NODE_ENV !== 'PROD') {
      warnings.forEach(w =>
        console.warn(
          `- Field "${w.path?.[w.path.length - 1]?.key}" is absent. Confirm expected behavior. \n\t- ${w.message}`,
        ),
      );
    }
  }
  ...
  if (errors.length) {
    throw new ValiError(
      errors as [BaseIssue<unknown>, ...BaseIssue<unknown>[]],
    );
  }
